...
public class AppConfig {
    ...
    SecurityWebFilterChain springSecurityFilterChain(ServerHttpSecurity http, ReactiveAuthenticationManager authenticationManager, ServerAuthenticationConverter authenticationConverter) {
        ...
        http.authorizeExchange(requests -> requests.pathMatchers(HttpMethod.POST, "/student").hasRole("Office").pathMatchers(HttpMethod.PUT, "/student/update").hasAnyRole("Office").pathMatchers(HttpMethod.GET, "/student/write-grades/").hasAnyRole("Office", "Teacher").pathMatchers(HttpMethod.GET, "/grades/my-grades").hasRole("Student").pathMatchers(HttpMethod.GET, "grades/*").hasRole("Teacher")
